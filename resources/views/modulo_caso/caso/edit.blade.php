    ...
    <div class="row">
        <div class="col-xs-12">
            <div class="box box-primary">
                ...
                <form role="form" action="{{ route('caso.update', $caso) }}" method="POST">
                    @method('PUT')
                    @csrf
                    <div class="box-body">
                        @if ($errors->any())
                        ...
                        <div class="form-group col-md-6 {{ ($errors->has('nombre_caso'))? 'has-error':'' }}">
                            ...
                            <input class="form-control" type="text" name="nombre_caso" value="{{ old('nombre_caso', $caso->nombre_caso) }}" required>
                        ...
                        <div class="form-group col-md-6 {{ ($errors->has('tipo_caso'))? 'has-error':'' }}">
                            ...
                            <select class="form-control select2" name="tipo_caso" required>
                                ...
                                <option value="{{ $tipocaso->id }}" @if ($caso->tipo_casos_id==$tipocaso->id) selected="selected" @endif>{{ $tipocaso->nombre }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group col-md-12 {{ ($errors->has('descripcion'))? 'has-error':'' }}">
                            ...
                            <textarea class="form-control" name="descripcion" rows="2" placeholder="Descripción Detallada del caso" required>{{$caso->descripcion}}</textarea>
                    ...
                    <div class="box-footer">
                        <div class="col-xs-6">
                            <a href="{{ route('etapacaso.index') }}" class="btn btn-default btn-block">Cancelar</a>
                        </div>
                        <div class="col-xs-6">
